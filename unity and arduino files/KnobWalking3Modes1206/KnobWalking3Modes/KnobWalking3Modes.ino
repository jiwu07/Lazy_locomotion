...
float AbsStepPace = 0;
float ExpAbsPhaseR = -PhaseWidths[0]/2;
float NextAbsPhaseR = ExpAbsPhaseR+PhaseWidths[1];
float ExpAbsPhaseL = -PhaseWidths[0]/2;
float NextAbsPhaseL = ExpAbsPhaseL+PhaseWidths[1];
...
void UpdatePhaseWidths(float w, float* ModeLength,float* PhaseWidths)
{
  ...
  for (int i = 0; i <= sizeof(ModeLength); i++)
  {
...
{
  Serial.print("Phases: ");
  for (int i=0;i<=sizeof(PhaseWidths);i++)
  {
    Serial.print(": ");
    Serial.print("Phase ");
    Serial.print(i);
